...
@section('title', 'Magserv | Editar Manutenção Preventiva')

@section('content')

    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Editar Manutenção Preventiva</h1>
            <p class="text-gray-600 mt-1">Altere os dados da manutenção preventiva com a OS
                {{ $manutencao->chamado }}, para o cliente {{ $manutencao->cliente->nome ?? 'N/A' }}</p>
        </div>
        <a href="{{ route('manutencoes.preventiva.index') }}"
    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
        <form action="{{ route('manutencoes.update', $manutencao->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    <label for="data_inicio_atendimento" class="block text-sm font-medium text-gray-700 mb-2">Data Início</label>
                    <input type="date" id="data_inicio_atendimento" name="data_inicio_atendimento" 
                           value="{{ old('data_inicio_atendimento', $manutencao->data_inicio_atendimento?->format('Y-m-d')) }}" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                    @error('data_inicio_atendimento') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="data_fim_atendimento" class="block text-sm font-medium text-gray-700 mb-2">Data Fim</label>
                    <input type="date" id="data_fim_atendimento" name="data_fim_atendimento" 
                           value="{{ old('data_fim_atendimento', $manutencao->data_fim_atendimento?->format('Y-m-d')) }}" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="flex justify-end mt-10 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="bg-blue-600 text-white hover:bg-blue-700 font-medium py-2 px-6 rounded-lg shadow-md">
                    Salvar Alterações
                </button>
            </div>
        </form>
